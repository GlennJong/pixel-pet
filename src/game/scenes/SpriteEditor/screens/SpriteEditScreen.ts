...
import { SpriteCard, CardField } from '../components/SpriteCard';
...
export class SpriteEditScreen extends Phaser.GameObjects.Container {
  ...
  private handleSelectModeKey(code: string) {
    switch (code) {
      ...
      case 'ArrowDown': {
        ...
        if (this.cursorIdx < max) { this.cursorIdx++; this.updateCardFocus(); this.scrollToCard(this.cursorIdx); }
        break;
      }
      case 'Space': {
  ...
  private handleFieldModeKey(code: string) {
    ...
    const CONFIG_FIELDS: CardField[] = ['name', 'freq', 'repeat', 'repeatDelay'];
    ...
    switch (code) {
      ...
      case 'ArrowLeft':
        ...
        break;
      case 'ArrowUp': {
        const idx = CONFIG_FIELDS.indexOf(this.focusedField);
        if (idx > 0) { this.focusedField = CONFIG_FIELDS[idx - 1]; this.updateFieldFocus(); }
        break;
      }
      case 'ArrowDown': {
        const idx = CONFIG_FIELDS.indexOf(this.focusedField);
        if (idx < CONFIG_FIELDS.length - 1) { this.focusedField = CONFIG_FIELDS[idx + 1]; this.updateFieldFocus(); }
        break;
      }
      case 'Space':
  ...
  private updateCardFocus() {
    ...
    this.cards.forEach((c, i) => {
      c.setCardFocus(i === this.cursorIdx && this.mode === 'card' && !this.selectMode);
